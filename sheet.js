...
function applyOperation() {
    ...
    const operationColumns = document.getElementById('operation-columns').value.toUpperCase().split(',');
    ...
    filteredData = data.filter(row => {
        ...
        return operationColumns.every(col => {
            const value = row[col];
            if (document.getElementById('operation').value === "null") {
                return value === null && (document.getElementById('operation-type').value === "or" ? true : primaryValue === null);
            } else {
                return value !== null && (document.getElementById('operation-type').value === "or" ? true : primaryValue !== null);
            }
        });
